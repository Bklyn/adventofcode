...
Error: That command requires priority 50. You currently have priority
0.  You must deposit 50 stars to increase your priority to the
required level.  The console flickers for a moment, and then prints
another message:
...
def turing(states, diag=12425180, init='A'):
    state = 'A'
    tape = [0]
    pos = 0
    for i in range(diag):
        # print(i, state, pos, len(tape))
        out, step, state = states[state][tape[pos]]
        tape[pos] = out
        pos += step
        if pos < 0:
            tape = [0] + tape
