...
def Generator(factor, val, mult=None):
    while True:
...
def judge(gens, steps=40000000, debug=False):
    GenA, GenB = gens
    result = quantify ([(x & 0xffff) == (y & 0xffff)
                        for x, y in zip (islice (GenA, steps),
                                         islice (GenB, steps))])
    return result

# assert judge((Generator (16807, 65), Generator (48271, 8921))) == 588

print (judge((Generator (16807, 699), Generator (48271, 124))))
print (judge((Generator (16807, 699, 4), Generator (48271, 124, 8)), steps=5000000))
