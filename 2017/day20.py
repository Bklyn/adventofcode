...
for line in Input(20):
    p, v, a = [vector(x[3:-1]) for x in line.strip().split(', ')]
...
P0 = list(Particles)
Accel = [p[2] for p in Particles]
slowest = min(Accel, key=lambda p: distance(p))
print (Accel.index(slowest))

# Collision detection
step = 0
while True:
    loc = Counter([p[0] for p in Particles])
    before = len(Particles)
    collisions = [p for p in Particles if loc[p[0]] > 1]
    Particles = [tick(p) for p in Particles if loc[p[0]] == 1]
    after = len(Particles)
    if after != before:
        print (step, after, after - before)
    step += 1
