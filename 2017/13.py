...
def calcsev(fw, time=0):
    sev = sum ([depth * range_
                for depth, range_ in enumerate(fw)
                if range_ and (time + depth) % (2 * (range_ - 1)) == 0])
    return sev

def cleanrun(fw, time):
    return not any ([
        range_
        for depth, range_ in enumerate(fw)
        if range_ and (time + depth) % (2 * (range_ - 1)) == 0])

assert calcsev ([3,2,0,0,4,0,4]) == 24
assert cleanrun ([3,2,0,0,4,0,4], 10) == True

layers = dict(array(Input(13)))
max_layer = max(layers.keys())
layers = [layers.get (x, 0) for x in range(max_layer+1)]

print (calcsev(layers))
time = 0
while not cleanrun (layers, time):
    time = time + 1
print (time)
