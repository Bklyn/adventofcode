...
def forms(m):
    ...
    for i in range(4):
        x = np.rot90 (m, k=i)
        for out in (x, np.fliplr(x)):
            tpl = tuple(out.flatten())
            if tuple (tpl) in seen:
                continue
            yield out
            seen.add (tpl)
...
def mstr(m, sep='/'):
    return sep.join ([''.join (['#' if x else '.' for x in row]) for row in m])
...
def generate_art(rules, rounds):
    ...
    for cycle in range(rounds):
        ...
        # print ('ROUND', cycle + 1, shape, Art.shape, '\n\t' + mstr (Art, sep='\n\t'))
        pass
    return Art
...
print (set_bits (generate_art (Rules, 18)))

# Spent a ton of time debugging this, but my error was checking for %3
# before %2.  Argh!
