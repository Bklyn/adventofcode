...
def count_orbits(parents, children, node, counts):
    if node not in counts:
        counts[node] = 0
        if node in parents:
            counts[node] = 1 + count_orbits(parents, children, parents[node], counts)
    return counts[node]


def get_chain(parents, node):
    chain = []
...
def find_santa(parents, children):
    assert "SAN" in parents
    assert "YOU" in parents
    # Find common ancestor
    mine = get_chain(parents, "YOU")
    his = get_chain(parents, "SAN")
    common = None
    for node in reversed(mine):
        if node in his:
            common = node
            break
    return len(mine) - mine.index(common) + len(his) - his.index(common) - 2
...
def parse_orbits(lines):
    ...
    for line in lines:
        inner, outer = line.strip().split(")", 1)
        assert outer not in parents
        parents[outer] = inner
        children[inner].add(outer)
    leaves = [node for node in parents.keys() if node not in children]
    counts = {}
    for leaf in leaves:
        count_orbits(parents, children, leaf, counts)
    dist = find_santa(parents, children)
    return sum(counts.values()), dist
...
if __name__ == "__main__":
    print(parse_orbits(Input(6).readlines()))
