...
def parse_spec(moves):
    path = {}
    loc = origin
    distance = 0
    for move in moves:
        direction, count = DIRS[move[0]], int(move[1:])
...
if __name__ == '__main__':
    p1, p2 = [line.strip() for line in Input(3).readlines()]
    p1 = parse_spec(p1.split(','))
    p2 = parse_spec(p2.split(','))
    intersect = set(p1.keys()) & set(p2.keys())
    print (min(cityblock_distance(p) for p in intersect))
    print (min(p1[p] + p2[p] for p in intersect))
