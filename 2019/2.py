...
def run(tape, debug=False):
    ...
    tape = tape[:]
    while tape[ip] != 99:
        insn, a, b, dest = tape[ip : ip + 4]
        if debug:
            print(
                "ip={} insn={} a={} b={} dest={} tape={}".format(
                    ip, insn, tape[a], tape[b], dest, tape
                )
            )
        if insn == 1:
            ...
            raise ValueError("Invalid instruction {} at ip={}".format(insn, ip))
        ip += 4
    return tape


assert run([1, 9, 10, 3, 2, 3, 11, 0, 99, 30, 40, 50])[0] == 3500
...
def reverb(tape, noun, verb):
    return [tape[0], noun, verb] + tape[3:]


if __name__ == "__main__":
    tape = list(vector(Input(2).read()))
    print(run(reverb(tape, 12, 2))[0])
    for noun in range(100):
        for verb in range(100):
            if run(reverb(tape, noun, verb))[0] == 19690720:
                print(100 * noun + verb)
                break
