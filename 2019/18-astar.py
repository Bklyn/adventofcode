...
def solve(maze, debug=False):
    ...
    def goal(state):
        ...
        return len(keys) - len(state.seen)

    def moves(state):
        open_doors = set(maze[p].upper() for p in state.seen)
        ...
        for n in neighbors4(state.pos):
            glyph = maze.get(n, "#")
            if not glyph in accessible:
                continue
            # print(state.pos, open_doors, n, maze.get(n))
            seen = state.seen
            if glyph in string.ascii_lowercase and n not in seen:
                seen = state.seen + tuple([n])
            yield State(n, seen)

    origin = next(p for p, at in maze.items() if at == "@")
    return Astar(State(origin, tuple()), moves, goal, debug=debug)
