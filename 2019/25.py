...
the command inv (for "inventory").  Extra spaces or other characters
aren't allowed - instructions must be provided precisely.
...
class Adventure(Intcode):
    def __init__(self, tape):
        super(Adventure, self).__init__(tape, input=[])
    ...
    def on_input(self):
        while not self.input:
            ...
            if line.startswith("#"):
                continue
            self.input.extend(ord(x) for x in line + "\n")
...
if __name__ == "__main__":
    tape = list(vector(Input(25).read()))
    game = Adventure(tape)
    game.run()
