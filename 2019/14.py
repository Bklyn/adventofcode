...
def parse(lines):
    ...
    for l in [l.strip() for l in lines]:
        ins, out = l.split(" => ")
        ins = dict(to_ple(x) for x in ins.split(", "))
        out, count = to_ple(out)
        assert out not in reactions
        reactions[out] = [count, ins]
    return reactions


def solve(reactions, fuel=1, debug=False):
    required = Counter(FUEL=fuel)
    stack = ["FUEL"]
    while stack:
        name = stack.pop()
        units, formula = reactions[name]
        need = required[name]
        rounds = math.ceil(need / units)
        if debug:
            print(
                "{}: need={} make={}*{}={} depth={}".format(
                    name, need, rounds, units, rounds * units, len(stack),
                )
            )
        for chem, count in formula.items():
            required[chem] += rounds * count
            if debug:
                print(
                    "\tchem={} need={}*{}={} required={}".format(
                        chem, rounds, count, rounds * count, required[chem]
                    )
                )
            if chem in reactions:  # Skip ORE
                stack.append(chem)
        required[name] -= rounds * units  # OK to go negative
    return required["ORE"]


def solve_ore(reactions, maxore=1000000000000):
    lo = 1
    hi = maxore
    while lo < hi:
        x = (lo + hi) // 2
        ore = solve(reactions, fuel=x)
        if ore > maxore:
            hi = x - 1
        else:
            lo = x
...
def check_ex(ex, expected):
    result = solve(parse(ex.strip().splitlines()))
...
assert solve(parse(EX3.strip().splitlines())) == 13312
...
assert solve(parse(EX5.strip().splitlines())) == 2210736


if __name__ == "__main__":
    reactions = parse(Input(14).readlines())
    print(solve(reactions, debug=True))
    print(solve_ore(reactions))
