...
class Springbot(Intcode):
    def __init__(self, tape, input=[], display=False):
        super(Springbot, self).__init__(tape, input=input)
...
def harness(tape, proglen=4, regs="ABCD", end="WALK"):
    insns = ["NOT", "OR", "AND"]
    oregs = ["T", "J"]
    regs = list(regs)
    all_insns = [" ".join(x) for x in itertools.product(insns, regs + oregs, oregs)]
    num_progs = factorial(len(all_insns)) // (
        factorial(proglen) * factorial(len(all_insns) - proglen)
    )
    for x, program in enumerate(itertools.combinations(all_insns, proglen)):
        input = [ord(c) for c in "\n".join(list(program) + [end + "\n"])]
        bot = Springbot(tape, input)
        print("\r", x + 1, "/", num_progs, end="")
        if bot.run() > 127:
            print("\n", bot.output[-1], program)
            break


if __name__ == "__main__":
    tape = list(vector(Input(21).read().strip()))
    # Part 1: brute force
    # harness(tape)
    part2 = (
        "\n".join(
    ...
    # harness(tape, proglen=6, regs="ABCDEFGHI", end="RUN")
    print(Intcode(tape, input=[ord(x) for x in part2]).run())
