...
def solve(asteroids):
    ...
    for ast in asteroids.keys():
        distances = asteroids[ast]
        for other in asteroids.keys():
            if ast == other:
                continue
            dist = (X(other) - X(ast), Y(other) - Y(ast))
            gcd = math.gcd(X(dist), Y(dist))
            z = (X(dist) // gcd, Y(dist) // gcd)
            distances[z].add(other)
            if len(distances) > len(most):
                most = distances
                best = ast
    ...
    def angle(pt):
        # return cmath.polar(complex(Y(pt), -X(pt)))[1]
        return -cmath.polar(complex(Y(pt), X(pt)))[1]

    laser = sorted(most.keys(), key=angle)
    for point in laser:
        most[point] = sorted(most[point], key=lambda p: distance(p, best))
        cplx = complex(Y(point), X(point))
        # print(best, point, angle(point), most[point])

    zapped = []
    while most:
        for point in laser:
            if point not in most:
                continue
            asts = most[point]
            zap = asts.pop(0)
            zapped.append(zap)
            # print(len(zapped), zap, 100 * X(zap) + Y(zap))
            if not asts:
                most.pop(point)

    return part1, best, zapped


def parse(lines):
    asteroids = {}
    for y, line in enumerate([l.strip() for l in lines]):
        for x, c in enumerate([c for c in line]):
            if c != ".":
                asteroids[(x, y)] = defaultdict(set)
    return asteroids
