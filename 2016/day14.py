...
consider the first such triplet in a hash.  One of the next 1000
hashes in the stream contains that same character five times in a row,
like 77777.
...
def solve (salt, numkeys=64, numhashes=1):
    keys = []
    matches = set()
    triples = []
    index = 0
    while len(keys) < numkeys:
        digest = do_hash (salt, index, numhashes)
        c = has_triple(digest)
        # print 'Round #%d: digest=%s c=%s triples=%d' % (
        # index, digest, c, len (triples))
        for idx, match in list(triples):
            if has_five(match, digest):
                # This digest matches an outstanding triple
        ...
        if len(triples) and triples[0][0] + 1000 == index:
            # print 'Age-out: triple=%s matches=%d iskey=%s' % (
            # triples[0], len (matches), triples[0][0] in matches)
            triples = triples[1:]
        if index - 1000 in matches:
