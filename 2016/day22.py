...
# sort by topology
nodes.sort (key=lambda n: (n[0][1], n[0][0]))
viable = 0
# Sorted by available space
vnodes = sorted (nodes, key=lambda n: n[3])
avail = [n[3] for n in vnodes]
...
def viable_nodes (n):
    ...
    v = [q for q in vnodes[x:] if q[0] != n[0]]
    return v
...
for n in nodes:
    ...
    if x == dims[0] and y == 0:
        ...
        rep = '_'
    elif n[1] < goal or len (viable_nodes (n)) == 0:
        rep = '#'
    else:
        rep = '.'
    if x < dims[0]:
