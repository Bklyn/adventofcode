...
def bounding_box(points):
    minx = min(X(p) for p, _ in points)
    maxx = max(X(p) for p, _ in points)
    miny = min(Y(p) for p, _ in points)
    maxy = max(Y(p) for p, _ in points)
...
def parse(lines):
    ...
    for line in lines:
        ...
        p, v = (int(match[1]), int(match[2])), (int(match[3]), int(match[4]))
        yield p, v
    pass


def dump(points):
    ul, lr = bounding_box(points)
    marks = set(p for p, _ in points)
    for y in range(Y(ul), Y(lr) + 1):
        for x in range(X(ul), X(lr) + 1):
            print ('#' if (x, y) in marks else ' ', end='')
        print ()
    print


def step(points):
    '''Evolve points by one step'''
    points = [((X(p) + X(v), Y(p) + Y(v)), v) for p, v in points]
    return points
...
def solve(points):
    ...
    while points:
        points = step(points)
        size = picture_size(points)
        steps += 1
        if size < min_size:
            min_size = size
            picture = points
        elif size > min_size:
            # The assumption is that the "picture" is formed when all
            # points are at their most densely packed (i.e. smallest
            # bounding box)
            break
    return (steps - 1, picture)


# steps, picture = solve(list(parse(example.read().splitlines())))
# dump(picture)
# print (steps, picture)

points = list(parse(Input(10).read()))
steps, picture = solve(points)
dump(picture)
print (steps)
