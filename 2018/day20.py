#!/usr/bin/env python3
'''
Advent of Code[About][Events][Shop][Settings][Log Out]Bklyn 36*
...
#, doors as | or -, your current position as X, and where north is up, the area you're in might look like this:
For example, drawing rooms as ., walls as
...
def solve(steps):
    ...
    for step in steps:
        if step in '^$':
            ...
            dx, dy = MOVES[step]
            pos = x + dx, y + dy
            pathlen = distance.get(prev, 0) + 1
            if pos in distance:
                pathlen = min(distance[pos], pathlen)
            distance[pos] = pathlen
    return distance.values()


lens = solve(Input(20).read().strip())
print(max(lens))
