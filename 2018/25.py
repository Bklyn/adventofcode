...
"Insufficient chronal energy for activation. Energy required: 50
stars."
...
def distance_4d(p, q):
    x1, y1, z1, w1 = p
    x2, y2, z2, w2 = q
    return abs(x2 - x1) + abs(y2 - y1) + abs(z2 - z1) + abs(w2 - w1)


points = array(Input(25))
distances = np.array([[distance_4d(p1, p2) for p2 in points] for p1 in points])
...
while todo:
    ...
    while True:
        sameconst = [idx
                     for p in const
                     for idx, x in enumerate(distances[p]) if x <= 3
                     if idx not in const]
        if not sameconst:
            break
        print(point, len(const), len(sameconst), sameconst)
        for idx in sameconst:
            const.add(idx)
            if idx in todo:
                todo.discard(idx)
