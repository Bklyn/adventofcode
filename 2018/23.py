...
nanobots. If there are multiple, choose one closest to your position
...
def parse(lines):
    ...
    for line in lines:
        v = vector(line.replace('pos=<', '').replace(
            '>', '').replace('r=', ''))
        yield v
...
def intersects(p1, r1, p2, r2):
    x1, y1, z1 = p1
    x2, y2, z2 = p2
    xleg = min(x1 + r1, x2 + r2) - max(x1 - r1, x2 - r2)
    yleg = min(y1 + r1, y2 + r2) - max(y1 - r1, y2 - r2)
    zleg = min(z1 + r1, z2 + r2) - max(z1 - r1, z2 - r2)
...
for (x, y, z, r) in parse(Input(23).read().strip()):
    bots[(x, y, z)] = r
    if r > max_power:
        max_power = r
        strongest = (x, y, z)

print(sum(1 for bot in bots if distance(bot, strongest) <= max_power))

bot_ranges = {}
for pos, r in bots.items():
    for pos2, r2 in bots.items():
        legs = intersects(pos, r, pos2, r2)
        if legs != (0, 0, 0):
            print(pos, r, pos2, r2, legs)
