...
register 2.  Opcode 9 could be seti: value 2 matches the value stored
in the output register, register 2; the number given for B is
irrelevant.
...
tests, at_least_three = 0, 0
opcode_candidates = defaultdict(set)
tape = []

for line in [line.strip() for line in Input(16).readlines()]:
    if line.startswith('Before: ['):
        before = vector(line[9:-1])
    elif line.startswith('After:  ['):
        after = vector(line[9:-1])
        opcode = args[0]
        candidates = 0
        for name, func in MACH.items():
            result = func(before, args[1:])
            if result == after[args[3]]:
                opcode_candidates[opcode].add(name)
                candidates += 1
            else:
        ...
        if candidates >= 3:
            at_least_three += 1
        assert candidates, (before, args, after)
        tape = []
    else:
        args = vector(line)
        if args:
...
while len(opcodes) != len(opcode_candidates):
    # We know these opcodes for sure
    discards = opcodes.values()
    for num, names in opcode_candidates.items():
        if num not in opcodes:
            # The known opcodes cannot be candidates for this number
            for name in discards:
                names.discard(name)
            if len(names) == 1:
                # We know this one for sure now
                opcodes[num] = first(names)

regs = [0] * 4
for args in tape:
    opcode, args = args[0], args[1:]
    func = MACH[opcodes[opcode]]
    regs[args[2]] = func(regs, args)
