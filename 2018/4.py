...
for line in map(str.strip, sorted(Input(4))):
    time = '20' + line[3:17]
    line = line[19:]
    ...
    if line.startswith('Guard'):
        ...
        continue
    elif line.startswith('falls asleep'):
        start_time = time
        continue
    assert time.minute >= start_time.minute
    # print(guard, start_time, time, (time - start_time).seconds // 60)
    minutes.update(range(start_time.minute, time.minute))
    for minute in range(start_time.minute, time.minute):
        Minutes[minute].update({guard: 1})
    Sleep.update({guard: time.minute - start_time.minute})

sleepy = Sleep.most_common()[0][0]
sleepiest_minute = Guards[sleepy].most_common()[0][0]
print(sleepy * sleepiest_minute)

most_asleep = 0
sleepiest_guard = None
peak_minute = None
for minute, guards in Minutes.items():
    guard, count = guards.most_common()[0]
    if count > most_asleep:
        sleepiest_guard, most_asleep = guard, count
        peak_minute = minute

print(sleepiest_guard * peak_minute)
