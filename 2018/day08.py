...
to the first child node, 2 to the second, 3 to the third, and so
on. The value of this node is the sum of the values of the child nodes
...
of node C is 0.  Node A has three metadata entries: 1, 1, and 2. The 1
references node A's first child node, B, and the 2 references node A's
second child node, C. Because node B has a value of 33 and node C has
a value of 0, the value of node A is 33+33+0=66.  So, in this example,
the value of the root node is 66.
...
def parse(iterable):
    nchild = next(iterable)
    nmeta = next(iterable)
    children = list(parse(iterable) for _ in range(nchild))
    return children, list(islice(iterable, nmeta))


def metadata(tree):
    children, meta = tree
    for child in children:
        yield from metadata(child)
    for item in meta:
        yield item
...
def value(node):
    ...
    for idx in meta:
        ...
        if idx < 0 or idx >= len(children):
            continue
        result += value(children[idx])
    return result


tree = parse(iter(vector('2 3 0 3 10 11 12 1 1 0 1 99 2 1 1 2')))
assert sum(metadata(tree)) == 138
assert value(tree) == 66

tree = parse(iter(vector(Input(8).read())))
