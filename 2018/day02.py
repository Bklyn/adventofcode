...
scores = [twos_and_threes(line) for line in Input(2)]
twos = sum(s[0] for s in scores)
threes = sum(s[1] for s in scores)
print(twos * threes)
...
for line in Input(2):
    ...
    for other in Input(2):
        other = other.strip()
        if line == other or len(line) != len(other):
            continue
        diffs = 0
        matches = []
        for i in range(len(other)):
            ...
            if diffs > 1:
                break
        if diffs == 1:
            print('{}\n{}\n{}'.format(line, other, ''.join(matches)))
            sys.exit(0)
