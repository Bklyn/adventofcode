...
def bb(coords):
    minx, miny, maxx, maxy = (min(X(c) for c in coords), min(Y(c) for c in coords),
                              max(X(c) for c in coords), max(Y(c) for c in coords))
    return (minx, miny), (maxx, maxy)


def bold(x):
    return '\033[1m{}\033[0m'.format(x)
...
def get_distances(point, coords):
    return [cityblock_distance(point, coord) for coord in coords]
...
def closest(distances):
    ...
    for idx, distance in enumerate(distances):
        if distance < winner[0]:
            winner = (distance, idx)
        elif distance == winner[0]:  # Tie
            winner = (distance, None)
    # print(point, winner, distances)
    return winner


def solve(coords, region=None, dump=False):
    bounding_box = bb(coords)
    (minx, miny), (maxx, maxy) = bounding_box
    nearest = Counter()
    candidates = set(range(len(coords)))
    region_size = 0
    # Any coordinate who is the winner at an edge is discarded
    for point in edges(bounding_box):
        distances = get_distances(point, coords)
        distance, idx = closest(distances)
        if idx is not None:
    ...
    for y in range(miny, maxy + 1):
        for x in range(minx, maxx + 1):
            point = (x, y)
            distances = get_distances(point, coords)
            if region is not None and sum(distances) < region:
                region_size += 1
            distance, idx = closest(distances)
            token = '.'
            if idx is not None:
                ...
                token = string.ascii_letters[idx % 52]
                if distance == 0:
                    token = bold(token)
            if dump:
                print(token, end='')
        if dump:
            print('')
    return nearest.most_common(), region_size


nearest, region = solve([(1, 1), (1, 6), (8, 3), (3, 4),
                         (5, 5), (8, 9)], region=32)
assert (nearest[0], region) == ((4, 17), 16)

coords = [vector(line) for line in Input(6)]
nearest, region = solve(coords, region=10000)
print(nearest[0], region)
